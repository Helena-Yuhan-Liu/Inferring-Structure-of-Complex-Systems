...
for iter=1:max_iter
    ...
    phi = rand*pi/2; 
    u(:,:,1)=tanh(sqrt(X.^2+Y.^2)).*cos(m*angle(X+i*Y)-(sqrt(X.^2+Y.^2)+phi));
    v(:,:,1)=tanh(sqrt(X.^2+Y.^2)).*sin(m*angle(X+i*Y)-(sqrt(X.^2+Y.^2)+phi));

%     num_coeff = 3;
%     start_coeff = 1;
%     ufft = zeros(length(x), length(y));
%     vfft = zeros(length(x), length(y));
%     ufft(start_coeff:start_coeff-1+num_coeff,start_coeff:start_coeff-1+num_coeff) = ...
%         repmat((rand(num_coeff,1)-.5) + (rand(num_coeff,1)-.5)*i, [1,num_coeff]);
%     vfft(start_coeff:start_coeff-1+num_coeff,start_coeff:start_coeff-1+num_coeff) = ...
%         repmat((rand(num_coeff,1)-.5) + (rand(num_coeff,1)-.5)*i, [1,num_coeff]);
